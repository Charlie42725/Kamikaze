        ...
        {isLoading && !data ? (
          ...
          settlements.map((settlement) => (
            <Card key={settlement.id} style={{ marginBottom: 12 }}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold">@{settlement.kol?.ig_handle || '未知'}</span>
              ...
              {settlement.notes && (
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-2 truncate">{settlement.notes}</div>
              )}
            </Card>
          ))
        )}
