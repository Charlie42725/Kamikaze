...
import { Card, List, Button, Dialog, Toast, Space } from 'antd-mobile';
...
export function AdminKolDetailClient({ kol, products, settlements, staff }: Props) {
    ...
    <div>
      ...
      <div className="p-4">
        <Card style={{ marginBottom: 16 }}>
          ...
          {staff && <div className="text-sm text-gray-500">負責人：{staff.display_name}</div>}
        </Card>
        ...
        {settlements.length > 0 && (
          <List header="結算紀錄">
            {settlements.map((s) => (
              <List.Item
                key={s.id}
                extra={[s.kol_amount != null ? `網紅 NT$${s.kol_amount}` : null, s.marketing_amount != null ? `行銷 NT$${s.marketing_amount}` : null].filter(Boolean).join(' / ') || '-'}
        ...
        {kol.notes && <List header="備註"><List.Item>{kol.notes}</List.Item></List>}

        <div style={{ padding: '24px 0' }}>
          <Space direction="vertical" style={{ width: '100%' }} block>
            <Button block color="primary" onClick={() => router.push(ROUTES.ADMIN.KOL_EDIT(kol.id))}>編輯網紅</Button>
            <Button
              block
              color="danger"
              fill="outline"
